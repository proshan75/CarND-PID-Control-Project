...
double PID::InitializeError(int num_of_errors)
{
    cout << __func__ << " _cte_total: " << _cte_total << " divided by: " << num_of_errors << endl;
    _best_error = this->_cte_total / num_of_errors;
    cout << "Error Initialized to: " << _best_error << endl;
    return _best_error;
}
...
void PID::Twiddle(double cte)
{
    ...
    {
        cout << " improvement 1 error " << error << " compared to " << _best_error;
        _best_error = error;
    ...
    else
    {
        ...
        else
        {
            ...
            {
                cout << " improvement 2 error " << error << " compared to " << _best_error;
                _best_error = error;
                cout << " changing _dp[Current_index] from " << _dp[Current_index];
                _dp[Current_index] *= 1.1;
                cout << " to " << _dp[Current_index] << " at index " << Current_index << endl;
            ...
            else
            {
                ...
                cout << " and reducing to " << _dp[Current_index];
            }
        }
    }
